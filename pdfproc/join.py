...
class PDFTable(table_base):
    ...
    def DeleteRow(self, row):
        ...
        if grid:
            ...
            grid.BeginBatch()
            msg = gridlib.GridTableMessage(
                self, gridlib.GRIDTABLE_NOTIFY_ROWS_DELETED, row, 1
            )
            grid.ProcessTableMessage(msg)
            grid.EndBatch()
...
class PDFDialog(wx.Dialog):
    def __init__(self, parent):
        ...
        self.btn_neu = wx.FilePickerCtrl(
            self,
            wx.ID_ANY,
            wx.EmptyString,
            "Select a PDF file",
        ...
        msg = wx.StaticText(
            self, wx.ID_ANY, msg_txt, defPos, wx.Size(-1, 50), wx.ALIGN_LEFT
        )
        msg.Wrap(-1)
        ...
        tx_ausdat = wx.StaticText(self, wx.ID_ANY, "Output:", defPos, defSiz, 0)
        szr03.Add(tx_ausdat, 0, wx.ALL | wx.ALIGN_RIGHT, 5)

        self.btn_aus = wx.FilePickerCtrl(
            self,
            wx.ID_ANY,
            ...
            defPos,
            wx.Size(480, -1),
            wx.FLP_OVERWRITE_PROMPT | wx.FLP_SAVE | wx.FLP_SMALL | wx.FLP_USE_TEXTCTRL,
        )
        szr03.Add(self.btn_aus, 0, wx.ALL, 5)

        tx_autor = wx.StaticText(self, wx.ID_ANY, "Author:", defPos, defSiz, 0)
        szr03.Add(tx_autor, 0, wx.ALIGN_RIGHT, 5)

        self.ausaut = wx.TextCtrl(
            self,
            wx.ID_ANY,
        ...
        pdf_titel = wx.StaticText(self, wx.ID_ANY, "Title:", defPos, defSiz, 0)
        szr03.Add(pdf_titel, 0, wx.ALIGN_RIGHT, 5)

        self.austit = wx.TextCtrl(
            self, wx.ID_ANY, "Joined PDF files", defPos, wx.Size(480, -1), wx.NO_BORDER
        )
        szr03.Add(self.austit, 0, wx.LEFT, 5)

        tx_subject = wx.StaticText(
            self, wx.ID_ANY, "Subject:", defPos, defSiz, wx.ALIGN_RIGHT
        )
        szr03.Add(tx_subject, 0, wx.ALIGN_RIGHT, 5)

        self.aussub = wx.TextCtrl(
            self, wx.ID_ANY, "Joined PDF files", defPos, wx.Size(480, -1), wx.NO_BORDER
        )
        szr03.Add(self.aussub, 0, wx.LEFT, 5)

        tx_keywords = wx.StaticText(self, wx.ID_ANY, "Keywords:", defPos, defSiz, 0)
        szr03.Add(tx_keywords, 0, wx.ALIGN_RIGHT, 5)

        self.keywords = wx.TextCtrl(
            self, wx.ID_ANY, " ", defPos, wx.Size(480, -1), wx.NO_BORDER
        )
        szr03.Add(self.keywords, 0, wx.LEFT, 5)
        tx_blank = wx.StaticText(self, wx.ID_ANY, " ", defPos, defSiz, wx.ALIGN_RIGHT)
        szr03.Add(tx_blank, 0, wx.RIGHT, 5)
        self.noToC = wx.CheckBox(
            self,
            wx.ID_ANY,
            "check to suppress table of content",
            defPos,
            defSiz,
            wx.ALIGN_LEFT,
        )
        ...
        szr04OK = wx.Button(self, wx.ID_OK, "SAVE", defPos, defSiz, wx.BU_EXACTFIT)
        szr04.AddButton(szr04OK)
        szr04Cancel = wx.Button(
            self, wx.ID_CANCEL, "QUIT", defPos, defSiz, wx.BU_EXACTFIT
        )
        ...
        # ==============================================================================
        linie1 = wx.StaticLine(self, wx.ID_ANY, defPos, defSiz, wx.LI_HORIZONTAL)
        linie2 = wx.StaticLine(self, wx.ID_ANY, defPos, defSiz, wx.LI_HORIZONTAL)
        linie3 = wx.StaticLine(self, wx.ID_ANY, defPos, defSiz, wx.LI_HORIZONTAL)
...
def make_pdf(dlg):
    ...
    for zeile in dlg.szr02.Table.data:
        ...
        pdf_out.insert_pdf(doc, from_page=von, to_page=bis, rotate=rot)
...
#
# ==============================================================================
assert (wx.VERSION[0], wx.VERSION[1], wx.VERSION[2]) >= (
    3,
    0,
    2,
