...
Then you are allowed to use it as:
...
if os.stat("%s\\bmarks.out" % workpath).st_size > 0:
    ...
    # Parse the sexpr
    pdfbmarks = walk_bmarks(sexpdata.load(open(workpath + r'\bmarks.out', encoding='utf-8-sig')), 0)
    ...
    for m in p.finditer(metadata):
        ...
        retval = retval | os.system(
            "pdftk %s update_info_utf8 %s output %s" % (args.dest, workpath + '\pdfmetadata.in', finaldest))
